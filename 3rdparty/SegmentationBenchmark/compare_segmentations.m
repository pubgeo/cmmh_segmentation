...
function [ri,gce,vi,cov]=compare_segmentations(sampleLabels1,sampleLabels2)
...
[cov] = covering_segmentation(sampleLabels2,sampleLabels1);
return;

function cov = covering_segmentation(i1,i2)
%i2 is groundtruth
    nsegs=length(unique(i1));
    labs1=unique(i1);
    nsegs2=length(unique(i2));
    labs2=unique(i2);
    regionsGT=[];
    cov=0;
    for s = 1 : nsegs
        p=find(i1==labs1(s));
        matches=zeros(1,nsegs2);
        for s2=1:nsegs2
            p2=find(i2==labs2(s2));
            in = intersect(p,p2);
            un = union(p,p2);
            if isempty(un)==1
                ...
                matches(s2)=length(in)/length(un);
            end
        end
        cov=cov+length(p)*max(matches);
    end
    cov=cov/(size(i1,1)*size(i1,2));
